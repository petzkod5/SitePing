...
export const ICON_PERSON = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`;
...
export type SortControlsChangeReason = "sort" | "groupByPage" | "filterByMe";
...
export class PanelSortControls {
  ...
  private _filterByMe = false;
  private menuEl: HTMLElement | null = null;
  private sortBtn: HTMLButtonElement;
  private groupToggle: HTMLButtonElement;
  private filterByMeToggle: HTMLButtonElement | null = null;
  private readonly t: TFunction;
  private readonly colors: ThemeColors;
  private readonly onChange: (reason: SortControlsChangeReason) => void;
  private readonly hostIdentity: SitepingIdentity | null;
  private outsideClickHandler: ((e: MouseEvent) => void) | null = null;

  constructor(
    colors: ThemeColors,
    onChange: (reason: SortControlsChangeReason) => void,
    t: TFunction,
    hostIdentity?: SitepingIdentity | null,
  ) {
    this.colors = colors;
    this.onChange = onChange;
    this.t = t;
    this.hostIdentity = hostIdentity ?? null;
    ...
    if (this.hostIdentity) {
      this.filterByMeToggle = document.createElement("button");
      this.filterByMeToggle.className = "sp-group-toggle sp-group-toggle--by-me";
      this.filterByMeToggle.setAttribute("aria-pressed", "false");
      this.filterByMeToggle.setAttribute("aria-label", this.t("group.byMe"));

      const personIcon = parseSvg(ICON_PERSON);
      this.filterByMeToggle.appendChild(personIcon);

      const byMeLabel = el("span", { class: "sp-group-toggle-label" });
      setText(byMeLabel, this.t("group.byMe"));
      this.filterByMeToggle.appendChild(byMeLabel);

      const filterByMeToggle = this.filterByMeToggle;
      filterByMeToggle.addEventListener("click", () => {
        this._filterByMe = !this._filterByMe;
        filterByMeToggle.classList.toggle("sp-group-toggle--active", this._filterByMe);
        filterByMeToggle.setAttribute("aria-pressed", String(this._filterByMe));
        this.onChange("filterByMe");
      });

      this.element.appendChild(this.filterByMeToggle);
    }
  }
  ...
  get filterByMe(): boolean {
    return this._filterByMe;
  }
